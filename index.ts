...
async function input() {
  ...
  const userinput: { userid: string, userpin: string } = await inquirer     //  userinput type is object in which only string is allowed
    .prompt([
  ...
  const selectopt: { options: string } = await inquirer
    .prompt([
      {
        name: "options",
  ...
  if
    (selectopt.options == "cashWithdrawl") {

    console.log("your current amount ", userdata.amount);

    const enteredAmount: { amount: number } = await inquirer
      .prompt([
        {
          name: "amount",
  ...
  (selectopt.options == "cashDeposit") {

    console.log("your current amount ", userdata.amount);

    const AddAmount: { Addamount: number } = await inquirer
      .prompt([
        {
          name: "Addamount",
